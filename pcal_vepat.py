...
class PcalsVepat(object):
    ...
    def phit_cal2(self):
        self.df1['Area'] = self.df1['square length'] ** 2
        self.df1['Phit_abv'] = math.pi * (
                    (self.df1['boulder diameter'] + 0.5 * self.df1['person diameter']) ** 2) .div(self.df1['Area'], axis = 0)
        self.df1['Phit_side'] = (2 * self.df1['boulder diameter'] + self.df1['person diameter']).div(
            self.df1['square length'], axis=0)
        # self.df1['Gmean'] = gmean(self.df1.iloc[:, 4:5], axis = 1)
        self.df1['Gmean'] = (self.df1['Phit_abv'] * self.df1['Phit_side']) ** (1 / 2)
        ...
        self.df1.columns = ['Boulder diameter (m)', 'Person diameter (m)', 'Square length (m)', 'Area (m^2)',
                            'P(hit) Above', \
                            'P(hit) Side', 'P(hit) Geometric mean', 'P(hit)']
        return self.df1
    ...
    def ballis_cal(self, df12):
        self.df3 = df12[['Boulder diameter (m)', 'P(hit)']]
